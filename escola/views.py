...
def alunos_view(request):
    alunos = Aluno.objects.prefetch_related('cursos').all().order_by('nome')
    return render(request, 'escola/alunos.html', {'alunos': alunos})

def curso_view(request, id):
    curso = get_object_or_404(Curso.objects.select_related('professor').prefetch_related('alunos'), id=id)
